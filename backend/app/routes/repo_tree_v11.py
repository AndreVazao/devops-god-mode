...
async def repo_tree_action_plan(repo_full_name: str):
    try:
        ...
        preview = await repo_tree_engine_v4.preview(repo_full_name, depth=2)
        if not preview.get("ok"):
            return latest

        embedded_snapshot = {
            "id": preview.get("cache", {}).get("snapshot_id"),
            "depth": preview.get("depth"),
            "analysis_status": preview.get("analysis_status"),
            "structural_hash": preview.get("cache", {}).get("structural_hash"),
            "frameworks": (preview.get("analysis") or {}).get("frameworks") or [],
            "repo_types": (preview.get("analysis") or {}).get("repo_types") or [],
            "risks": (preview.get("analysis") or {}).get("risks") or [],
            "recommendations": (preview.get("analysis") or {}).get("recommendations") or [],
        }
        action_plan = repo_tree_action_plan_v2.build_from_embedded_snapshot(repo_full_name, embedded_snapshot)
        action_plan["fallback_used"] = True
        action_plan["fallback_reason"] = latest.get("reason", "latest_snapshot_unavailable")
        return action_plan
    except Exception as exc:
        return {
            "ok": False,
