...
router = APIRouter(
    prefix="/api/offline-command-buffering",
    tags=["offline-command-buffering"],
)
...
async def update_offline_command_connectivity(payload: ConnectivityUpdateRequest):
    return offline_command_buffering_service.set_connectivity(
        pc_online=payload.pc_online,
        phone_online=payload.phone_online,
    )
...
async def mark_offline_command_execution(
    command_id: str,
    payload: CommandExecutionUpdateRequest,
):
    try:
