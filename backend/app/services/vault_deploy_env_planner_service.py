...
DEPLOY_ENV_STORE = AtomicJsonStore(DEPLOY_ENV_FILE, default_factory=lambda: {"project_env_manifests": {}, "secret_presence": {}, "readiness_reports": []})
...
class VaultDeployEnvPlannerService:
    ...
    def get_status(self) -> Dict[str, Any]:
        return {"ok": True, "mode": "vault_deploy_env_planner_status", "status": "vault_deploy_env_planner_ready", "planner_file": str(DEPLOY_ENV_FILE), "atomic_store_enabled": True}
    ...
    def _normalize_store(self, store: Dict[str, Any]) -> Dict[str, Any]:
        ...
        store.setdefault("project_env_manifests", {})
        store.setdefault("secret_presence", {})
        store.setdefault("readiness_reports", [])
        return store

    def _load_store(self) -> Dict[str, Any]:
        return self._normalize_store(DEPLOY_ENV_STORE.load())

    def _save_store(self, store: Dict[str, Any]) -> None:
        DEPLOY_ENV_STORE.save(self._normalize_store(store))
    ...
    def _classify_env(self, env_name: str) -> Dict[str, Any]:
        ...
        return {"env_name": env_name, "is_public": is_public, "is_secret": is_secret, "storage_target": "project_env" if is_public else "secret_vault"}

    def register_secret_presence(self, secret_name: str, provider: str = "unknown", scope: str = "owner-andre", present: bool = True, tenant_id: str = "owner-andre") -> Dict[str, Any]:
        normalized_name = secret_name.strip().upper()
        if not normalized_name:
            return {"ok": False, "error": "secret_name_empty"}
        key = f"{tenant_id}:{scope}:{normalized_name}"
        record = {"secret_name": normalized_name, "provider": provider, "scope": scope, "tenant_id": tenant_id, "present": bool(present), "updated_at": self._now()}
        ...
        DEPLOY_ENV_STORE.update(mutate)
        return {"ok": True, "mode": "vault_deploy_env_register_secret_presence", "secret": record}

    def _build_manifest_from_graph(self, project_id: str, tenant_id: str) -> Dict[str, Any]:
        ...
        for repo in project.get("repositories", []):
            ...
            repo_manifests.append({"repo_id": repo.get("repo_id"), "repository_full_name": repo.get("repository_full_name"), "roles": repo.get("roles", []), "deploy_targets": repo.get("deploy_targets", []), "stack": repo.get("stack", []), "required_env": [self._classify_env(env) for env in required_env]})
        manifest = {
    ...
    def build_project_env_manifest(self, project_id: str, tenant_id: str = "owner-andre") -> Dict[str, Any]:
        manifest_result = self._build_manifest_from_graph(project_id=project_id, tenant_id=tenant_id)
    ...
    def build_readiness_report(self, project_id: str, tenant_id: str = "owner-andre") -> Dict[str, Any]:
        ...
        report = {
            ...
            "blockers": blockers,
            "approval_steps": [
                {"step_id": "confirm-secret-binding", "label": "Confirmar que os secrets corretos podem ser ligados aos providers de deploy", "approval_required": True},
                {"step_id": "prepare-provider-env-sync", "label": "Preparar sincronização para Vercel/Render/Supabase/GitHub Actions sem expor valores", "approval_required": True},
            ],
            "manifest": manifest,
        }
    ...
    def build_dashboard(self, tenant_id: str = "owner-andre") -> Dict[str, Any]:
        ...
        latest_by_project = {report["project_id"]: report for report in reports}
        return {"ok": True, "mode": "vault_deploy_env_dashboard", "tenant_id": tenant_id, "manifest_count": len(manifests), "secret_presence_count": len(secret_records), "readiness_report_count": len(reports), "blocked_project_count": len([item for item in latest_by_project.values() if item.get("status") != "ready_for_env_binding"]), "manifests": manifests, "recent_reports": reports, "secret_presence": secret_records}

    def seed_demo_baribudos_secrets(self, tenant_id: str = "owner-andre") -> Dict[str, Any]:
        seeded = []
        for secret_name, provider in [("GITHUB_TOKEN", "github"), ("VERCEL_TOKEN", "vercel"), ("VERCEL_ORG_ID", "vercel"), ("SUPABASE_URL", "supabase"), ("SUPABASE_ANON_KEY", "supabase")]:
