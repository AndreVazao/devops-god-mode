...
class GitHubService:
    ...
    async def _get_json(self, client: httpx.AsyncClient, url: str, params: dict[str, Any] | None = None) -> Any:
    ...
    async def _fetch_content(self, client: httpx.AsyncClient, full_name: str, path: str) -> dict[str, Any] | None:
        response = await client.get(f"{GITHUB_API_BASE}/repos/{full_name}/contents/{path}", headers=self._headers(), timeout=20.0)
    ...
    async def _detect_common_files(self, client: httpx.AsyncClient, full_name: str) -> dict[str, bool]:
        found: dict[str, bool] = {}
    ...
    async def _fetch_languages(self, client: httpx.AsyncClient, full_name: str) -> dict[str, int]:
        return await self._get_json(client, f"{GITHUB_API_BASE}/repos/{full_name}/languages")

    async def _fetch_recent_commits(self, client: httpx.AsyncClient, full_name: str, per_page: int = 5) -> list[dict[str, Any]]:
        commits = await self._get_json(client, f"{GITHUB_API_BASE}/repos/{full_name}/commits", params={"per_page": per_page})
        return [{
            "sha": item.get("sha"),
    ...
    def _classify_stack(self, package_json_text: str | None, common_files: dict[str, bool], languages: dict[str, int]) -> dict[str, Any]:
        ...
        if package_json_text:
            try:
                ...
                dependencies = {**(parsed.get("dependencies") or {}), **(parsed.get("devDependencies") or {})}
                scripts = parsed.get("scripts") or {}
            except json.JSONDecodeError:
                pass

        if common_files.get("requirements.txt") or common_files.get("pyproject.toml") or "Python" in languages:
            runtimes.append("python")
        if package_json_text or common_files.get("next.config.mjs") or common_files.get("next.config.js") or "TypeScript" in languages or "JavaScript" in languages:
        ...
        return {"runtimes": sorted(set(runtimes)), "frameworks": sorted(set(frameworks)), "scripts": scripts}

    def _to_hybrid_summary(self, repo: dict[str, Any], stack: dict[str, Any], common_files: dict[str, bool]) -> dict[str, Any]:
        ...
        return {"plain": plain, "technical": stack}
    ...
    async def scan_repositories(self, limit: int = 20) -> dict[str, Any]:
        ...
        async with httpx.AsyncClient() as client:
            ...
            for repo in repositories[:limit]:
                ...
                scanned.append({**repo, "languages": languages, "common_files": common_files, "stack": stack, "recent_commits": recent_commits, "summary": self._to_hybrid_summary(repo, stack, common_files)})
        return {"ok": True, "count": len(scanned), "repositories": scanned}
