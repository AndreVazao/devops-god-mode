...
class InstallReadinessFinalGateService:
    ...
    def build_check(self, tenant_id: str = "owner-andre", requested_project: str = "GOD_MODE", run_deep: bool = True) -> Dict[str, Any]:
        ...
        rehearsal = self._safe("real_operator_rehearsal", lambda: real_operator_rehearsal_service.run(tenant_id=tenant_id, requested_project=requested_project))
        ...
        checks = [
            ...
            self._check("apk_artifact_not_placeholder", "APK esperado não parece placeholder", workflow_probe.get("apk", {}).get("ok") is True, workflow_probe.get("apk")),
            self._check("exe_artifact_not_placeholder", "EXE esperado não parece placeholder", workflow_probe.get("exe", {}).get("ok") is True, workflow_probe.get("exe")),
            ...
            self._check("memory_sensitive_guard_active", "Guarda de memória sensível está ativa", self._memory_guard_ok(memory), memory.get("secret_probe")),
            ...
            self._check("backend_autonomy_contract", "Backend continua sem APK até OK/intervenção/bloqueio/fim", autonomy_probe.get("ok") is True, autonomy_probe),
            self._check("home_exposes_install_readiness", "Home expõe Instalação final", self._home_action_ok(home), self._home_action(home)),
        ]
    ...
    def _general_test_ok(self, panel: Dict[str, Any], rehearsal: Dict[str, Any]) -> bool:
        ...
        return bool(button and button.get("label") == "Teste geral" and button.get("endpoint") == "/api/real-operator-rehearsal/run" and rehearsal.get("status") == "ready" and rehearsal.get("score") == 100)
    ...
    def _artifacts_ok(self, artifacts: Dict[str, Any]) -> bool:
        ...
        return bool(artifacts.get("status") == "ready" and artifacts.get("artifact_count") == 2 and {"apk", "exe"}.issubset(artifact_ids) and {"godmode-android-webview-apk", "godmode-windows-exe"}.issubset(expected_names) and {"GodModeMobile-debug.apk", "GodModeDesktop.exe"}.issubset(expected_files))
    ...
    def _frontend_probe(self) -> Dict[str, Any]:
        ...
        text = path.read_text(encoding="utf-8") if path.exists() else ""
        markers = ["renderResultCard", "resultCard", "Ver JSON", "runPayloadButton(endpoint,payload={})", "r?.score", "r?.flow?.route_result?.job_id"]
    ...
    def _tree_probe(self) -> Dict[str, Any]:
        ...
        return {"ok": not missing, "missing_entries": missing, "required_entries": self.REQUIRED_TREE_ENTRIES, "generated_line_count": len(generated_tree.splitlines()), "source": "Project Tree Autorefresh generated tree"}

    def _memory_guard_ok(self, memory: Dict[str, Any]) -> bool:
        probe = memory.get("secret_probe", {}) if isinstance(memory, dict) else {}
        return bool(probe.get("blocked") is True and probe.get("safe_write_ok") is True)
    ...
    def _autonomy_probe(self, panel: Dict[str, Any], rehearsal: Dict[str, Any]) -> Dict[str, Any]:
        ...
        contract = {
            ...
            "allowed_stop_reasons": ["operator_ok_required", "manual_action_required", "safe_blocked", "finished"],
        ...
        return {"ok": bool(rule_ok and stop_reason_present), "operator_rules_contain_contract": rule_ok, "rehearsal_has_autopilot_stop_reason": stop_reason_present, "observed_stop_reason": route_summary.get("autopilot_stop_reason"), "contract": contract}

    def _home_action(self, home: Dict[str, Any]) -> Dict[str, Any] | None:
        for action in home.get("quick_actions", []) if isinstance(home, dict) else []:
            if action.get("id") == "install_readiness_final":
                return action
        return None

    def _home_action_ok(self, home: Dict[str, Any]) -> bool:
        action = self._home_action(home)
        return bool(action and action.get("endpoint") == "/api/install-readiness-final/check")
    ...
    def get_status(self, tenant_id: str = "owner-andre") -> Dict[str, Any]:
        ...
        return {"ok": check["ok"], "mode": "install_readiness_final_status", "status": check["status"], "score": check["score"], "ready_to_install": check["ready_to_install"], "failed_count": len(check["failed_checks"]), "operator_next": check["operator_next"]}

    def get_package(self, tenant_id: str = "owner-andre", requested_project: str = "GOD_MODE") -> Dict[str, Any]:
        return {"ok": True, "mode": "install_readiness_final_package", "package": {"status": self.get_status(tenant_id=tenant_id), "check": self.build_check(tenant_id=tenant_id, requested_project=requested_project, run_deep=True)}}
