...
INVENTORY_STORE = AtomicJsonStore(INVENTORY_FILE, default_factory=lambda: {"conversations": [], "project_map": {}})
...
class MultiAIConversationInventoryService:
    ...
    def _normalize_inventory(self, inventory: Dict[str, Any]) -> Dict[str, Any]:
        ...
        inventory.setdefault("conversations", [])
        inventory.setdefault("project_map", {})
        return inventory

    def _load_inventory(self) -> Dict[str, Any]:
        return self._normalize_inventory(INVENTORY_STORE.load())

    def _save_inventory(self, inventory: Dict[str, Any]) -> None:
        INVENTORY_STORE.save(self._normalize_inventory(inventory))
    ...
    def _normalize_provider(self, provider: str | None) -> str:
        ...
        aliases = {"chat gpt": "chatgpt", "openai": "chatgpt", "google gemini": "gemini", "bard": "gemini", "xai": "grok", "deep seek": "deepseek"}
    ...
    def _extract_repo_roles(self, text: str) -> List[str]:
        ...
        return sorted({role for role, terms in role_terms.items() if any(term in lowered for term in terms)})

    def _extract_languages(self, text: str) -> List[str]:
        lowered = text.lower()
        languages = [language for language in ["python", "typescript", "javascript", "kotlin", "java", "swift", "sql", "html", "css", "yaml"] if language in lowered]
    ...
    def _summarize(self, title: str, snippet: str) -> str:
        base = re.sub(r"\s+", " ", (snippet or title).strip().replace("\n", " "))
        return (base[:237].rstrip() + "...") if len(base) > 240 else (base or "Sem resumo disponível.")

    def stage_conversation(self, provider: str, title: str, snippet: str = "", conversation_url: str | None = None, project_hint: str | None = None, tenant_id: str = "owner-andre", source_status: str = "manual_seed") -> Dict[str, Any]:
        ...
        project_entry_result: Dict[str, Any] = {}

        def mutate(inventory: Dict[str, Any]) -> Dict[str, Any]:
            nonlocal project_entry_result
            inventory = self._normalize_inventory(inventory)
            inventory["conversations"].append(record)
            project_entry = inventory["project_map"].setdefault(project["project_id"], {"project_id": project["project_id"], "project_name": project["project_name"], "conversation_ids": [], "providers": [], "repo_roles": [], "languages": [], "last_seen_at": created_at})
            ...
            project_entry_result = project_entry
            return inventory

        INVENTORY_STORE.update(mutate)
        return {"ok": True, "mode": "multi_ai_conversation_stage", "conversation": record, "project_entry": project_entry_result}

    def seed_from_operator_command(self, command_id: str | None = None, tenant_id: str = "owner-andre") -> Dict[str, Any]:
        ...
        staged = self.stage_conversation(provider="unknown", title=f"Operator command: {command.get('intent', 'general')}", snippet=command.get("raw_text", ""), conversation_url=None, project_hint=project_name, tenant_id=tenant_id, source_status="operator_command_seed")
        return {"ok": True, "mode": "multi_ai_seed_from_operator_command", "source_command": command, "staged": staged}

    def list_conversations(self, tenant_id: str = "owner-andre", project_id: str | None = None, provider: str | None = None, limit: int = 100) -> Dict[str, Any]:
        ...
        return {"ok": True, "mode": "multi_ai_conversation_list", "tenant_id": tenant_id, "project_id": project_id, "provider": provider, "conversation_count": len(conversations), "conversations": conversations}

    def build_project_map(self, tenant_id: str = "owner-andre") -> Dict[str, Any]:
        inventory = self._load_inventory()
        project_map = sorted(list(inventory.get("project_map", {}).values()), key=lambda item: item.get("last_seen_at", ""), reverse=True)
        return {"ok": True, "mode": "multi_ai_project_map", "tenant_id": tenant_id, "project_count": len(project_map), "projects": project_map}
    ...
    def build_inventory_dashboard(self, tenant_id: str = "owner-andre") -> Dict[str, Any]:
        ...
        for item in conversations:
            ...
            if item.get("mapping_status") != "mapped":
                needs_review += 1
        return {"ok": True, "mode": "multi_ai_conversation_inventory_dashboard", "tenant_id": tenant_id, "conversation_count": len(conversations), "project_count": len(inventory.get("project_map", {})), "provider_counts": provider_counts, "needs_review_count": needs_review, "projects": self.build_project_map(tenant_id=tenant_id).get("projects", []), "recent_conversations": conversations[-20:]}

    def get_package(self) -> Dict[str, Any]:
        return {"ok": True, "mode": "multi_ai_conversation_inventory_package", "package": {"status": self.get_status(), "dashboard": self.build_inventory_dashboard()}}
